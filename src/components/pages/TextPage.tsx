...
export default function TextPage({ config, content, embedded = false }: TextPageProps) {
    return (
        <div className={`${embedded ? "" : "max-w-3xl mx-auto"} ${config.print_title ? "printable-page" : ""}`}>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                {config.print_title && !embedded && (
                    <button
                        ...
                        className="rounded py-2 text-sm font-medium text-accent underline decoration-accent/40 underline-offset-2 hover:decoration-accent focus:outline-none focus-visible:ring-2 focus-visible:ring-accent print:hidden"
                    >
                        Print / Save as PDF
                    </button>
                )}
